...
def call_workflow(api_url: str, payload: Dict[str, Any]) -> Dict[str, Any]:
    """POST to /jobs and poll /jobs/{job_id} until completed."""
    ...
    while True:
        ...
        if events:
            # show progress using the latest event
            last_event = events[-1]
            agent = last_event.get("agent", "system")
            placeholder.info(f"⏳ Status: **{status}** | Processing: `{agent}`")
            
        if status == "completed":
            placeholder.empty()
            # Flatten state into the result dict for the UI
            final_data = data["result"].copy()
            if "state" in final_data:
                final_data.update(final_data.pop("state"))
            return final_data
        elif status == "error":
            placeholder.empty()
...
def render_workflow_timeline(state: Dict[str, Any]) -> Set[str]:
    """Render the end-to-end workflow stages from the demo diagram."""
    stage_map = [
        ("Input", bool(state.get("resume_text") and state.get("job_description")), {"resume_text", "job_description"}),
        ("Analyzer", any(k in state for k in ["role", "seniority", "company", "matching_skills", "initial_match_score"]), {"role", "seniority", "company", "tech_stack", "matching_skills", "missing_skills", "nice_to_have_skills", "strengths", "weaknesses", "keyword_matches", "keyword_gaps", "ats_score", "initial_match_score"}),
        ("Rewriter", any(k in state for k in ["rewritten_resume", "rewritten_bullet_points", "cover_letter"]), {"rewritten_resume", "rewritten_bullet_points", "cover_letter"}),
        ("Critic", any(k in state for k in ["critic_score", "critic_feedback", "detected_errors", "weak_phrasing"]), {"critic_score", "critic_feedback", "detected_errors", "weak_phrasing", "rewrite_iteration"}),
        ("Interview", any(k in state for k in ["interview_questions", "technical_questions", "gap_questions", "preparation_tips", "key_topics_to_review", "expected_questions"]), {"interview_questions", "technical_questions", "gap_questions", "preparation_tips", "key_topics_to_review", "expected_questions"}),
    ]

    active = sum(1 for _, is_done, _ in stage_map if is_done)
    total = len(stage_map)
    progress = int((active / total) * 100) if total else 0

    st.markdown("### 🔄 Workflow Overview")
    st.progress(progress, text=f"{active}/{total} stages complete")

    cols = st.columns(len(stage_map))
    for col, (label, is_done, _) in zip(cols, stage_map):
        with col:
            status = "✅" if is_done else "⏳"
            col.markdown(
                f"<div style='border:1px solid rgba(127,127,127,0.25); border-radius:10px; padding:0.75rem; background:rgba(127,127,127,{0.04 if is_done else 0.02});'>"
                f"<b>{status} {label}</b></div>",
                unsafe_allow_html=True,
            )

    return {key for _, _, keys in stage_map for key in keys if key in state}


def render_rewriter_output(state: Dict[str, Any]) -> Set[str]:
    """Render the rewritten resume and bullet points."""
    rewrite_keys = {"rewritten_resume", "rewritten_bullet_points"}
    if not any(k in state and state[k] not in (None, "", []) for k in rewrite_keys):
        return set()

    st.markdown("### ✍️ Resume Rewrite")

    if state.get("rewritten_resume"):
        st.text_area(
            "Rewritten resume",
            value=str(state["rewritten_resume"]),
            height=260,
            disabled=True,
        )

    if state.get("rewritten_bullet_points"):
        st.markdown("**Tailored bullet points**")
        render_items(state["rewritten_bullet_points"])

    return rewrite_keys & set(state.keys())


def render_cover_letter(state: Dict[str, Any]) -> Set[str]:
    """Render the generated cover letter in a dedicated section."""
...
def render_critic_output(state: Dict[str, Any]) -> Set[str]:
    """Render critic scoring, feedback, and issue lists."""
    critic_keys = {"critic_score", "critic_feedback", "detected_errors", "weak_phrasing", "rewrite_iteration"}
    if not any(k in state and state[k] not in (None, "", []) for k in critic_keys):
        return set()
    ...
    if state.get("critic_score") is not None:
        cols = st.columns(2)
        cols[0].metric("Critic score", f"{state['critic_score']}/10")
        if state.get("rewrite_iteration") is not None:
            cols[1].metric("Rewrite iteration", state["rewrite_iteration"])

    if state.get("critic_feedback"):
        st.markdown("**Feedback**")
        render_items(state["critic_feedback"])

    if state.get("detected_errors"):
        st.markdown("**Detected errors**")
        render_items(state["detected_errors"])

    if state.get("weak_phrasing"):
        st.markdown("**Weak phrasing**")
        render_items(state["weak_phrasing"])

    return critic_keys & set(state.keys())


def render_cover_letter(state: Dict[str, Any]) -> Set[str]:
    """Backward-compatible alias for the rewrite section."""
    return render_rewriter_output(state)
...
if "last_result" in st.session_state:
    ...
    consumed_keys |= render_workflow_timeline(result)
    consumed_keys |= render_analyzer_output(result)
    consumed_keys |= render_rewriter_output(result)
    consumed_keys |= render_cover_letter(result)
    consumed_keys |= render_critic_output(result)
